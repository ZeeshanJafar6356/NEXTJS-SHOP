...
const CouponCard: React.FC<CouponCardProps> = ({ coupon, className }) => {
  ...
  return (
    <div className={cn('coupon-card', className)}>
      ...
      <div className="rounded-be mx-auto grid w-11/12 auto-cols-fr grid-flow-col items-center rounded-bl bg-light py-4 px-5 shadow-sm">
        {is_valid ? (
          <>
            ...
            {!copyText.copied && (
              ...
              >
                <button className="text-sm font-semibold text-accent transition-colors duration-200 hover:text-accent-hover focus:text-accent-hover focus:outline-none ltr:text-right rtl:text-left">
